...
CREATE TABLE #TempTableFinalResult
	...
	DECLARE database_cursor CURSOR FOR
		...
		WHERE dbid > 5 AND 
			  dbid NOT IN (19) AND 
			  name = 'DocumentManagement_Yesterday' 
...
DECLARE database_cursor CURSOR FOR
	SELECT DBName,TableName,ColumnName,character_maximum_length,TABLESCHEMA FROM #TempTable
OPEN database_cursor FETCH NEXT
FROM database_cursor INTO @DBName1,@tableName,@columnName ,@character_maximum_length ,@schemaname               
WHILE @@FETCH_STATUS = 0 
			BEGIN
			SET @Command1 =  'insert into #TempTableFinalResult(DBName, SchemaName,[TableName], ColumnName, [MaxLength], [RowCount],character_maximum_length) 

			                  SELECT top 10000 '''+ @DBName1 + ''','''+ @schemaname + ''','''+ @tableName + ''', ''['+ @columnName + ']'', MAX(LEN(['+@columnName+'])) AS [MaxLength] ,COUNT(*) AS [RowCount], '''+ @character_maximum_length + '''From '+@DBName1+'.'+@schemaname+'.['+@tableName +'] Order by 1 desc'
